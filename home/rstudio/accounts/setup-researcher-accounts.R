...
if (!file.exists("accounts/credentials.csv")) {
  students <- readLines("accounts/researchers.csv")
  ...
  users$Login <- login
  users$Pass <- pass
  write.csv(users, file = "accounts/credentials.csv", row.names = FALSE)  
}

users <- read.csv("accounts/credentials.csv")
...
write(bash_add, file = "accounts/user_add.sh")
write(bash_del, file = "accounts/user_del.sh")
